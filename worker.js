/**
 * Trip Splitter – Cloudflare Worker
 * Short clean links: https://your-worker.workers.dev/t/ABC123
 *
 * Setup:
 * 1. Create a KV namespace named TRIP_STORE in Cloudflare dashboard
 * 2. Bind it to this worker as TRIP_STORE
...
function shortLink(){
  return location.origin + '/t/' + state.id;
}
  ...
  async fetch(request, env) {
    ...
    if (path === '/api/trips' && request.method === 'POST') {
      try {
        ...
        body.id = id;
        await env.TRIP_STORE.put('trip:' + id, JSON.stringify(body), { expirationTtl: 60 * 60 * 24 * 90 }); // 90 days
    ...
    // Short link redirect page (serve the app)
    ...
    // Static assets (icons) – return 404 so browser falls back, or you can upload them separately
